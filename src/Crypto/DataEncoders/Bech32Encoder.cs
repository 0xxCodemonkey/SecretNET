...
internal class Bech32FormatException : FormatException
{
    internal Bech32FormatException(string message, int[] indexes) : base(message)
    {
        if (indexes == null)
    ...
    internal int[] ErrorIndexes
    {
        get; set;
    }
}
internal class Bech32EncodingType
{
    ...
    internal readonly static Bech32EncodingType BECH32;
    internal readonly static Bech32EncodingType BECH32M;
    internal readonly static Bech32EncodingType[] All;
    internal Bech32EncodingType(int encodingConstant)
    {
        EncodingConstant = encodingConstant;
    }
    internal int EncodingConstant { get; }
}
internal class Bech32Encoder
{
    ...
    internal Bech32Encoder(byte[] hrp)
    {
        if (hrp == null)
    ...
    internal byte[] HumanReadablePart
    {
        get
        {
            return _Hrp;
    ...
    internal virtual string EncodeData(ReadOnlySpan<byte> data, Bech32EncodingType encodingType)
    {
    ...
    internal static Bech32Encoder ExtractEncoderFromString(string test)
    {
    ...
    internal byte[] DecodeDataRaw(string encoded, out Bech32EncodingType encodingType)
    {
        return DecodeDataCore(encoded, out encodingType);
    }
    internal bool StrictLength { get; set; } = true;
    ...
    internal virtual byte[] ConvertBits(ReadOnlySpan<byte> data, int fromBits, int toBits, bool pad = true)
    {
        var acc = 0;
        var bits = 0;
        var maxv = (1 << toBits) - 1;
    ...
    internal virtual byte[] Decode(string addr, out byte witnessVerion)
    {
        if (addr == null)
    ...
    internal string EncodeRaw(byte[] data, Bech32EncodingType encodingType)
	{
		return EncodeData(data.AsSpan(), encodingType);
	}
    internal string EncodeRaw(ReadOnlySpan<byte> data, Bech32EncodingType encodingType)
	{
		return EncodeData(data, encodingType);
	}

    internal string Encode(byte witnessVerion, byte[] witnessProgramm)
	{
    ...
    internal string Encode(byte witnessVerion, ReadOnlySpan<byte> witnessProgramm)
	{
		if (witnessVerion > 16)
