...
public readonly struct GE
{
    ...
    public readonly FE x, y;

    public readonly bool infinity; /* whether this represents the point at infinity */
    ...
    {
        return new GE(x.NormalizeVariable(), y, infinity);
    }
    [MethodImpl(MethodImplOptions.AggressiveInlining)]
    ...
    public static bool TryParse(ReadOnlySpan<byte> pub, out bool compressed, out GE elem)
    {
        ...
        if (pub.Length == 33 && (pub[0] == SECP256K1_TAG_PUBKEY_EVEN || pub[0] == SECP256K1_TAG_PUBKEY_ODD))
        {
            ...
            return
                ...
                TryCreateXOVariable(x, pub[0] == SECP256K1_TAG_PUBKEY_ODD, out elem);
        }
