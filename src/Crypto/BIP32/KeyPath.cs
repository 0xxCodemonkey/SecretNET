...
internal class KeyPath
{
    ...
    /// <returns></returns>
    internal static KeyPath Parse(string path)
    {
    ...
    internal static bool TryParse(string path, out KeyPath? keyPath)
    {
        if (path == null)
    ...
    internal static KeyPath FromBytes(byte[] data)
    {
        if (data == null)
    ...
    internal KeyPath(params uint[] indexes)
    {
    ...
    internal KeyPath Derive(int index, bool hardened)
    {
        if (index < 0)
    ...
    internal KeyPath Derive(string path)
    {
        return Derive(new KeyPath(path));
    }

    internal KeyPath Derive(uint index)
    {
        return Derive(new KeyPath(index));
    }

    internal KeyPath Derive(KeyPath derivation)
    {
        return new KeyPath(
            _Indexes
